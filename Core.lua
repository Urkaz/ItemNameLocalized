...
INL.AttachItemTooltip = function(self, index)
	if not INL.requireReload then
		...
		if link then
			...
			id = tonumber(id)
			if id then
				itemIndex = INL.FindIndex(tonumber(id),1,table.maxn(INL.items))
				if itemIndex then
					if INL.settings.showTooltipLine then
						INL.AddTooltipLine(self, INL.items[itemIndex][2], color, "enUS:")
					end
					...
					if INL.settings.showTooltipLine then
						INL.AddTooltipLine(self, L["Missing locale"] .. " #".. id, "ffff0000", INL.settings.selectedLocale .. ":")
					end
					if INL.settings.showTooltipTitle then
						INL.AppendTooltipTitle(self, "|cffff0000" .. L["Missing locale"] .. " #".. id .."|r", index)
					end
				end
			end
		end
